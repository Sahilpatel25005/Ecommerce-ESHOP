...
function Cart() {
  ...
  return (
    <>
      <div
        className={` w-[100vw] h-full sm:w-[25vw] bg-white top-0 z-50 right-0 fixed sm:rounded-tl-md sm:rounded-bl-md p-6 ${
          isCartOpen ? "translate-x-0" : "translate-x-full"
        } transition-all duration-500 `}
      >
        <button
          className="bg-gray-200 p-2 rounded-[50%] top-2 right-2 fixed text-center w-[44px] text-2xl font-bold"
          onClick={() => dispatch(setIsopen())}
        >
          X
        </button>
        <div className="mt-8">
          <h1 className="text-black text-2xl font-medium text-center sm:text-3xl">
